...
export function highlightCode(code: string, lang: string): Promise<string> {
	...
	const promise = getHighlighter().then(async (highlighter) => {
		...
		if (effectiveLang !== "text") {
			await ensureLang(highlighter, effectiveLang as ShikiBundledLang);
		}
		...
		<Suspense
			fallback={
				<div className="group/code relative mb-2">
					<div className="overflow-x-auto rounded-lg bg-surface-1 p-3 text-xs">
						{lines.map((line, i) => (
							...
							<div key={i} style={{ height: "1.25rem" }}>
								{line.trim() && (
									<span
										...
										style={{
											width: `${Math.min(Math.max(line.length, 3), 60)}ch`,
											height: "0.7rem",
											marginTop: "0.2rem",
										}}
									/>
